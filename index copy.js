...
async function regenerateImage(container) {
    ...
    if (settings.useEcg) {
        await generateWithECG(currentgenType, currentPrompt, ch_name, charDialogue, (finalUrl, error) => {
            if (error) {
                console.error(`${LOG_PREFIX} [ECG Mode] Generation failed:`, error);
                return;
            }
            console.log(`${LOG_PREFIX} [ECG Mode] Generation successful. Final URL: ${finalUrl}`);
...
async function handleEcgGeneration(messageId, message) {
    const messageElement = $(`#chat [mesid="${messageId}"]`);
    const messageTextElement = messageElement.find('.mes_text');
    //get the value of ch_name from messageElement
    const ch_name = messageElement.attr('ch_name') || 'Unknown';
    ...
    // 诊断日志 3: 输出提取结果
    console.log(`${LOG_PREFIX} [ECG Mode] Regex matched. Type: ${genType}, Prompt: ${prompt}`);
    ...
    try {
        ...
        await generateWithECG(genType, finalPrompt, ch_name, charDialogue, (finalUrl, error) => {
            if (error) {
                console.error(`${LOG_PREFIX} [ECG Mode] Generation failed:`, error);
                return;
            }
            console.log(`${LOG_PREFIX} [ECG Mode] Generation successful. Final URL: ${finalUrl}`);
            finishGeneration(finalUrl, finalPrompt, messageId, genType);
        });
 ...
 */
async function generateWithECG(genType, prompt, characterName, charDialogue, onComplete) {
    const settings = getSettings();
 ...
 */
async function finishGeneration(finalUrl, prompt, messageId, genType) {
    const settings = getSettings();
    const context = getContext();
    const message = context.chat[messageId];
    const messageElement = $(`#chat [mesid="${messageId}"]`);
    ...
    if (settings.useCustomContainers) {
        const imageData = {
            ...
            genType: genType,
            timestamp: Date.now(),
            settings: { // Store the settings used for this specific generation
                aspectRatio: `${settings.ecgStaticWidth}:${settings.ecgStaticHeight}`, // Example, can be more detailed
        ...
        const container = createCustomImageContainer(finalUrl, prompt, settings);
        container.attr('data-image-id', imageData.id);
        ...
        // const hiddenSpanHtml = fullMatch.replace(/>.*?</g, '><');
        // messageTextElement.html(originalHtml.replace(fullMatch, hiddenSpanHtml));
...
async function handleLegacyGeneration(messageId, message) {
    ...
    if (extractionResult) {
        ...
        const originalImagePrompt = extractionResult.prompt;
        // const fullMatch = extractionResult.fullMatch;
            ...
            if (imageUrl) {
                ...
                if (settings.useCustomContainers) {
                    const imageData = {
                        ...
                        genType: extractionResult.genType || 'txt2img',
                        timestamp: Date.now(),
                        settings: {
                ...
                const generating_status = `/echo timeout=10000 ${t`Generating image failed`}`;
                await executeSlashCommandsWithOptions(generating_status);
            }
        } catch (error) {
            const generating_status = `/echo timeout=10000 ${t`Generating image failed`}`;
